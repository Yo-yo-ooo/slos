...
/* 一般调用rcc的命令行格式是: rcc -target=x86/linux [选项,例如: -v -s] [输入文件,例如:/tmp/lcc19400.i] [输出文件,例如:/tmp/lcc19401.s] */
/*int main(int argc, char *argv[])*/
void umain(int argc, char **argv) {
	int i, j;
	for (i = argc - 1; i > 0; i--) /* 用一个循环来查找选项 -target=xxxx */
		if (strncmp(*argv[i], "-target=", 8) == 0)
			break;
	if (i > 0) { /* 如果i>0,说明找到了形为 -target=xxxx 的选项 */
		char *s = strchr(*argv[i], '\\'); /* 找反斜杠字符 */
		if (s != NULL) /* 若用户提供的命令行字串里有反斜杠\ */
			*s = '/'; /* 把反斜杠(\)转化为正斜杠(/) */
		for (j = 0; bindings[j].name && bindings[j].ir; j++)
			if (strcmp(&argv[i][8], bindings[j].name) == 0) {
				IR = bindings[j].ir; /* 对于x86/linux字串,IR指向了在文件x86linux.md中被定义的类型为Interface的结构体变量 x86linuxIR */
				break;
			}
		if (s != NULL) /* 若之前曾有反斜杠\ */
			*s = '\\'; /* 则再将其还原为正斜杠/ */
	}
	if (!IR) { /* 若没找到与命令行上指定的架构对应的后端Interface结构 */
		fprint(stderr, "%s: unknown target", *argv[0]); /* 打印rcc程序名和警告信息 */
		if (i > 0) /* 若在命令行上找到了形如-target=xxxx的选项,但此xxxx选项中指定的架构不被lcc所支持 */
			fprint(stderr, " `%s'", &argv[i][8]); /* 打印此xxxx架构的字串 */
		fprint(stderr, "; must specify one of\n");
		for (i = 0; bindings[i].name; i++) /* 打印lcc所支持的架构列表 */
