...
static struct entry {
	...
	struct entry *link; /* 指向下一个entry */
} *typetable[128];

/**
 * maxlevel的值是类型表(typetable)中类型所在符号表types中有相关入口的
 * 类型的u.sym->scope 的最大值.
 */
static int maxlevel;

/**
 * 指向"指针类型"的符号结构(Symbol结构)的指针
 * Hints: "T*"
 */
...
/**
 * type - 在typetable中搜索指定类型或创建一个新的类型.
 * @op: 类型操作符编码
 * @ty: 类型操作数
 * @size: 大小
 * @align: 对齐字节
	...
	if (op != FUNCTION && (op != ARRAY || size > 0))
		for (tn = typetable[h]; tn; tn = tn->link)
			...
			&&  tn->type.u.sym == sym)
				return &tn->type;
	/*
	 * 若找不到要搜索的指定类型或者要创建/查找
	 * 的是数组或函数类型则创建新类型.
	 * type函数总是为函数类型和不完全的数组类型创建新类型.
	 */
	NEW0(tn, PERM);
	tn->type.op = op;
	tn->type.type = ty;
...
/**
 * rmtpyes - 从类型表(typetable)中删除那些u.sym->scope 大于或等于lev的类型.
 * @lev: 指定层数
 * 返回值: 无
 */
...
/**
 * ptr - 创建一个指向ty类型的指针类型,并将其添加至类型表里,并返回指针.
 * @ty: 原类型
 * 返回值: 指针类型(指向原类型)
 */
Type ptr(Type ty) {
...
/**
 * deref - 解引用一个指针类型(比如一个int *类型,解引用后就变成了int类型了)
 * @ty: 指针类型
 * 返回值: 解引用后的类型
 */
Type deref(Type ty) {
 ...
 * @a: 数组元素类型对象的字节大小
 * 返回值: 返回的数组类型
 * NOTE: 如果a等于0,那么数组的对齐大小就是数组中元素的对齐大小
 */
Type array(Type ty, int n, int a) {
	...
	} else if (n > INT_MAX/ty->size) { // 如果数组元素数量太多
...
/**
 * atop - 将数组类型转化成指向数组元素的指针类型(比如: 数组作为形参时)
 * @ty: 数组类型
 * 返回值: 指向数组元素的指针类型
 */
Type atop(Type ty) {
	if (isarray(ty))
		return ptr(ty->type); // ty->type 是数组元素的类型
	error("type error: %s\n", "array expected");
	return ptr(ty);
}
...
/**
 * qual - 为ty类型加上限定符(修饰符)
 * @op: 类型限定符: const,volatile,const+volatile
 * @ty: 要限定的类型
 * 返回值: 被限定过的类型
 */
Type qual(int op, Type ty) {
	...
	||       isvolatile(ty) && op == VOLATILE)
		...
		/**
		 * 代码走到这里只可能有两种情况:
		 * 1. op == CONST , ty->op == VOLATILE
		 * 2. op == VOLATILE , ty->op == CONST
		 */
		if (isqual(ty)) {
			op += ty->op;
			ty = ty->type;
 ...
 * func - 创建一个函数类型
 * @ty: 返回值类型
 * @proto: 参数类型列表(指向以空指针结尾的Type数组)
 * @style: 新式的函数定义还是旧式的函数定义(1表示旧风格,0表示新风格)
 * 返回值: 新创建的函数类型
 */
...
/**
 * variadic - 可变参数断言函数(测试一个函数类型是否有可变参数列表)
 * @ty: 要测试的函数类型
 * 返回值: 1,该函数有可变参数列表. 0,该函数没有可变参数列表
 */
...
/**
 * newstruct - 创建一个struct类型或union类型
 * @op: 有两个取值: STRUCT 或者 UNION
 * @tag: 结构体标签,比如: struct xxoo { int a; int b; }; 则xxoo为这个结构体的tag
 * 返回值: 类型指针
 */
Type newstruct(int op, char *tag) {
	...
	/* 如果找到了符号 且 scope等于当前level
	 * 或者
	 * 如果找到了符号 且 scope是参数作用域 且 当前level等于PARM+1(也就是局部变量作用域)
	 * 因为ANSI C规定参数和局部变量的作用域相同
	 */
	else
		if ((p = lookup(tag, types)) != NULL && (p->scope == level
		|| p->scope == PARAM && level == PARAM+1)) {
			// 如果该符号的类型码等于新创建的struct or union类型码,且没有被定义过.
			// 换句话说就是找到了类型声明.
	...
	if (p->scope > maxlevel)
		maxlevel = p->scope;
	p->src = src; // 将当前的坐标信息赋值给新创建的符号的坐标域
	return p->type;
}

/**
 * newfield - 在struct或union类型里添加一个新域
 * @name: 要添加的域的名字(类型成员名),注意: name 可能为NULL,因为struct支持匿名位域.
 * @ty: struct或union的类型
 * @fty: 成员的类型
 * 返回值: 新域成员的指针
 */
 ...
 * @ty1: 类型1
 * @ty2: 类型2
 * @ret: 如果两个类型是不完全类型,则拿着ret的值返回
 * 返回值: 如果两个类型兼容,返回1.否则返回0.
 */
int eqtype(Type ty1, Type ty2, int ret) {
	if (ty1 == ty2) /* 如果两个type指针的地址相同,则直接返回1 (TRUE) */
		return 1;
	if (ty1->op != ty2->op) /* 如果两个类型的op域不同则返回0 (FLASE) */
		return 0;
	/* 程序执行到此,(ty1 != ty2) && (ty1->op == ty2->op) */
	switch (ty1->op) {
	...
	case ARRAY:    if (eqtype(ty1->type, ty2->type, 1)) {
		       	if (ty1->size == ty2->size)
		       		return 1;
		       	/* 如果两个数组类型之一是不完全数组类型,则返回ret的值 */
		       	if (ty1->size == 0 || ty2->size == 0)
		       		return ret;
		       }
		       return 0;
	case FUNCTION: if (eqtype(ty1->type, ty2->type, 1)) { /* 比较函数返回值的类型 */
				/* 下面开始比较函数的参数列表的类型是否相同 */
		       	Type *p1 = ty1->u.f.proto, *p2 = ty2->u.f.proto;
		       	/* 如果两个函数的原型数组的首地址相同(两个函数是同一函数) */
		       	if (p1 == p2)
		       		return 1;
		       	/* 依次比较函数的每一个参数的类型 */
		       	if (p1 && p2) {
				...
				if (*p1 == NULL && *p2 == NULL)
					return 1;
		       		...
		       		/**
		       		 * 进入到else分支后,p1和p2中至少有一个为NULL
		       		 * 如果任何一个函数原型有可变参数,则返回不兼容
		       		 * 如果p1不为0,则p2一定为0;如果p1为0,ty1一定没有可变参数
		       		 * 且p2可能为0,所以就判断ty2是否有可变参数
		       		 * */
...
/***
 * isfield - 判断name是否在flist中,并返回field结构
 * @name: 要判断的结构域名(field name)
 * @flist: 结构的域名列表
 * 返回值: 如果name在flist中,则返回该域名结构的首地址,否则返回NULL.
 */
